    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 pt-16">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <Link to="/" className="inline-flex items-center space-x-2 text-purple-400 hover:text-purple-300 mb-8 transition-colors">
            <ArrowLeft className="h-4 w-4" />
            <span>Back to Home</span>
          </Link>

          <div className="space-y-8">
            <h1 className="text-4xl md:text-6xl font-bold text-white">Ready for VR 180°?</h1>
    ...
    <div className="relative w-full h-screen bg-black">
      {showControls && (
        <div className="absolute top-4 left-4 right-4 z-50 flex justify-between items-center">
          <Link to="/" className="bg-black/50 backdrop-blur-sm text-white px-4 py-2 rounded-lg flex items-center space-x-2 hover:bg-black/70 transition-colors">
            <ArrowLeft className="h-4 w-4" />
            <span>Exit VR</span>
          </Link>

          <div className="flex items-center space-x-2">
            <button onClick={toggleMute} className="bg-black/50 backdrop-blur-sm text-white p-2 rounded-lg hover:bg-black/70 transition-colors">
              {isMuted ? <VolumeX className="h-5 w-5" /> : <Volume2 className="h-5 w-5" />}
            </button>
            <button onClick={resetView} className="bg-black/50 backdrop-blur-sm text-white p-2 rounded-lg hover:bg-black/70 transition-colors">
              <RotateCcw className="h-5 w-5" />
            </button>
            <button onClick={enterFullscreen} className="bg-black/50 backdrop-blur-sm text-white p-2 rounded-lg hover:bg-black/70 transition-colors">
      ...
      <a-scene embedded style={{ width: '100%', height: '100%' }} vr-mode-ui="enabled: true" device-orientation-permission-ui="enabled: true">
        ...
        <a-camera look-controls="enabled: true; reverseMouseDrag: false" wasd-controls="enabled: false" position="0 1.6 0">
          <a-cursor
        ...
        <a-text value="Look around to explore the 180° experience" position="0 -2 -5" align="center" color="white" width="8"></a-text>
      </a-scene>
      ...
      <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 text-center text-white bg-black/50 backdrop-blur-sm px-6 py-3 rounded-lg">
        <p className="text-sm">Click and drag to look around • Press VR button for headset mode</p>
      </div>
    </div>
  );
};
